...
public class SensorEventHelper implements SensorEventListener {
	...
	public void onSensorChanged(SensorEvent event) {
		...
		case Sensor.TYPE_ORIENTATION: {
		    //取值范围是0-360 ，顺时针逐渐增大，getScreenRotationOnPhone基本都是返回0，除非APP界面变换为横屏 颠倒屏，才有值，不会具体旋转值
			float x = event.values[0];
			Log.d("map","event.values[0]=="+x+"     getScreenRotationOnPhone"+"===="+getScreenRotationOnPhone(mContext));
			...
			Log.d("map","mAngle=="+mAngle+"x===="+x);
			mAngle = Float.isNaN(x) ? 0 : x;
			if (mMarker != null) {
			    //marker 旋转角度是正时是逆时针旋转
